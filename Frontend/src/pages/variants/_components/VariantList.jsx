...
import { Icon, Table, TableBody, TableCell, TableHead, TableHeaderCell, TableRow, Title } from '@tremor/react'
...
function VariantList ({ productCopy, navigate, id, deleteProductVariant }) {
  return (
    <Table>
      ...
      <TableBody>
        {productCopy.variants !== undefined
          ? productCopy.variants.map((variant) => {
            return (
              <TableRow key={variant.id} className='hover:bg-slate-800 hover:cursor-pointer'>
                ...
                <TableCell onClick={(e) => { navigate('/products/' + id + '/variants/' + variant.id + '/edit') }}>
                  ...
                  <small>{variant.tax}% - {currencyFormatter(variant.price * variant.tax)}</small><br />
