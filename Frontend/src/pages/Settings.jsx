...
function Settings () {
  ...
  const createCategoryAction = async () => {
    ...
    if (response) setLocalCategories((old) => [...old, { id: response, name, products: 0 }])
  }

  const deleteCategoryAction = async (category) => {
    const response = await deleteCategory(category.id)
    if (response) setLocalCategories((old) => old.filter((c) => c.id !== category.id))
  }

  const updateCategoryAction = async (category) => {
    const response = await updateCategory(category.id, category.title)
    if (response) setLocalCategories((old) => old.map((c) => c.id === category.id ? category : c))
  }
  ...
  return (
    <SideMenu>
      ...
      >
        ...
        <TextInput readOnly value={settings.siteName} onChange={(e) => setSettings({ ...settings, siteName: e.target.value })} className='my-2' placeholder='Nombre de la tienda' />
        <TextInput readOnly value={settings.siteDescription} onChange={(e) => setSettings({ ...settings, siteDescription: e.target.value })} className='my-2' placeholder='Descripcion de la tienda' />
        <TextInput readOnly value={settings.siteUrl} onChange={(e) => setSettings({ ...settings, siteUrl: e.target.value })} className='my-2' placeholder='URL de la tienda' />
      </ContainerCard>
      ...
      <Grid numItems={1} numItemsSm={2} className='mt-2 gap-2'>
        ...
        <Card>
          ...
          <TextInput readOnly value={settings.postHog.projectId} onChange={(e) => setSettings({ ...settings, postHog: { ...settings.postHog, projectId: e.target.value } })} className='my-2' placeholder='PostHog Project ID' />
          <TextInput readOnly value={settings.postHog.apiKey} onChange={(e) => setSettings({ ...settings, postHog: { ...settings.postHog, apiKey: e.target.value } })} className='my-2' placeholder='PostHog Public Key' />
          <TextInput readOnly value={settings.postHog.host} onChange={(e) => setSettings({ ...settings, postHog: { ...settings.postHog, host: e.target.value } })} className='my-2' placeholder='PostHog API Key' />
          <Divider />
          <Button
            variant='secondary'
          ...
          <Button
            ...
            className='my-2 w-full' color='amber' variant='secondary'
          >Crear flags requeridos
          </Button>
